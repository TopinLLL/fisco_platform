...
func mockAmend(ctx *gin.Context) {
	time.Sleep(time.Second * 5)
	response.Success(ctx, nil, fmt.Sprintf("%s%s", "修正区块数分别为0,20,40,60,80,100时实验数据如下（单位ms）:",
		"传统安全多方计算方案:0,55.2,111.1,167.4,225.3,279.8"+
			"无权限者组随机选择用户方案:"+
			"0,47.3,92.5,137.5,185.3,231.1"))
}
...
func mockPrivacy(ctx *gin.Context) {
	time.Sleep(time.Second * 5)
	response.Success(ctx, nil, fmt.Sprintf("%s%s", "电子合同数量为0，50，100，150，200，250，300时成功率如下:",
		"方案35:0,27.7,51.2,69.9,81.4"+
			"方案36:0,39.5,63.4,86.61,95.1"+
			"本文方案：0,91.7,95.6,97.3,99.3"))
}

func mockAttackAmend(ctx *gin.Context) {
	time.Sleep(time.Second * 6)
	response.Success(ctx, nil, fmt.Sprintf("%s%s", "作恶节点分别为1，10，20，40时成功率如下:",
...
func mockPrivacyDoor(ctx *gin.Context) {
	time.Sleep(time.Second * 5)
