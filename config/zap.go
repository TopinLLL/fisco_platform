...
func init() {
	if ok := common.PathExists(global.GlobalConfig.Zap.Director); !ok { // 判断是否有Director文件夹
	...
	//NewTee是往多个core中写数据
	Logger = zap.New(zapcore.NewTee(cores[:]...), zap.WithCaller(false))
	fmt.Println(global.GlobalConfig.Zap.ShowLine)
...
func getEncoderCore(fileName string, level zapcore.LevelEnabler) (core zapcore.Core) {
	writer := common.GetWriteSyncer(fileName) // 使用file-rotatelogs进行日志分割
	return zapcore.NewCore(getEncoder(), writer, level)
}

// 自定义日志输出时间格式
func CustomTimeEncoder(t time.Time, enc zapcore.PrimitiveArrayEncoder) {
	enc.AppendString(t.Format(global.GlobalConfig.Zap.Prefix + "2006/01/02 - 15:04:05.000"))
}
